...
./testHTML.exe &>testHTML.log 
mv testHTML.log ../log

echo

echo "Testing HTML INPUT"
echo 

#compare the output found to our expected
#if -l was set, print expected
if cmp -s testHTML.html ../tests/testHTML.data > /dev/null ; then
	echo -e "testCase 1   ---------------------------------------------------------\033[30m\033[42m  PASS  \033[0m"
else
	if [ "$listing" -eq "1" ] ; then
		echo "testCase 1 - expected output is:"
		cat ../tests/testHTML.data
	fi
